...
public class CommentServiceImpl implements CommentService {
    ...
    public void deleteCommentById(Integer id) {
        // 使用 deleteById() 方法根据 ID 删除，参数为 Integer 类型的 id
    ...
    public Comment saveCommentAndChangeCounts(Comment comment) {
        ...
        int count = 1;
        Article a = articleRepository.findById(comment.getArticle().getId())
                .orElseThrow(() -> new NoSuchElementException("Article not found with id: " + comment.getArticle().getId()));        a.setCommentCounts(a.getCommentCounts() + count);
    ...
    public void deleteCommentByIdAndChangeCounts(Integer id) {
        ...
        Comment c = commentRepository.findById(id)
                .orElseThrow(() -> new NoSuchElementException("Comment not found with id: " + id));        Article a = c.getArticle();
